...
def api_selecttrainsession(request):
    if request.GET.get('datea') and request.GET.get('dateb') and request.GET.get('properties') and request.GET.get(
            'details'):
...
def api_setiplist(request):
    ...
    if params.get('isdel'):
        if params.get('isdel') == 'true':
            ...
            if params.get('ip') and params.get('type') and params.get('label') and params.get('time') and params.get(
                    'istrash'):
...
# 查一天的数据
def api_top(request):
    if request.GET.get('type') and request.GET.get('date') and request.GET.get('limit') and request.GET['type'] in [
        'order', 'query', 'onlyquery']:
