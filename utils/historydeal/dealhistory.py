...
CREATE TABLE IF NOT EXISTS `crawlerip` (
...
class MakeTable(object):
    ...
    def __init__(self):
        ...
        self.class2list = ['KMG','XIY','CKG','HGH','XMN','NKG','WUH','CSX','URC','TAO','CGO','SYX','HAK','TSN','DLC','HRB','KWE','SHE','FOC','NNG']
    ...
    def maindeal(self,datea):
        ...
        print( 'ten AND crawleri over!')
        print( time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
        self.MakeIpPieData(datea)
        print( 'Pie over!')
        self.route_count_daily(datea)
        print( 'route_count_daily over!')
        print( time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
        self.makeipmessage(datea)
        print( 'makeipmessage over!')
    ...
    def getdata(self,a):   #先把数据取出来，便于提高效率
        ...
        while(date1 < dateend):
            ...
            AND {querytime} < '{date2}'
            '''.format(error = const.TABLE_ER,ip = const.TABLE_IP,de=const.TABLE_DE, ar=const.TABLE_AR,table=const.RAWFF,querytime = const.TABLE_QUERYTIME, date=date1.strftime(timeformat), date2 = date2.strftime(timeformat)))
            ...
            WHERE {querytime} >= '{date}'
            AND {querytime} < '{date2}'
            '''.format(ssmessage=const.TABLE2_ROUTE , ip = const.TABLE_IP, table=const.RAWSS,querytime = const.TABLE_QUERYTIME, date=date1.strftime(timeformat), date2 = date2.strftime(timeformat)))
            result = self.cursor.fetchall()
            for i in range(len(result)):
                if result[i][0] == None:
                    buycount -= 1
    ...
    def route_count_daily(self,a):
        ...
        class2list = ['KMG','XIY','CKG','HGH','XMN','NKG','WUH','CSX','URC','TAO','CGO','SYX','HAK','TSN','DLC','HRB','KWE','SHE','FOC','NNG']
        topiplist = []
    ...
    def MakeIpPieData(self,a):  #绘制第一张饼图数据的据
        ...
        for i in range(5):
            for j in range(5):
                ...
                self.cursor.execute(sql1,(a,rangenamelist[j],len(np.unique(alllist[i][j]['all'])),len(np.unique(alllist[i][j]['buy'])),len(np.unique(alllist[i][j]['crawler'])),i+1))
                self.cursor.execute(sql2,(a,rangenamelist[j],len(alllist[i][j]['all']),len(alllist[i][j]['buy']),len(alllist[i][j]['crawler']),i+1))
        for i in range(5):
            querylist = []
            orderlist = []
            crawlerlist = []
            ...
            self.cursor.execute(sql1,(a,rangenamelist[i],len(np.unique(querylist)),len(np.unique(orderlist)),len(np.unique(crawlerlist)),6))
        alllist = []
        return 0
    def makeipmessage(self,dataa):     #处理部分ip的信息表，用于第四个页面
        ...
        while date1 < dateend:
            ...
            sql = 'INSERT INTO procdata.iphourmessage ( ip,time, query,buy) VALUES ( %s, %s, %s,%s)'
            self.cursor.execute(sql,('all',date1.strftime(timeformat),self.hourindex[index+1] - self.hourindex[index],self.hourbuyindex_buy[index+1]-self.hourbuyindex_buy[index]))
            hourcache = {}
        ...
        for i in range(len(iplist)):
            ...
            sql = 'INSERT INTO procdata.ipdaymessage ( ip,time, duration, query, buy, depature, arrival, variance, mean, error,hotcity,normalcity) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s)'
            self.cursor.execute(sql,(iplist[i],dataa,w_durantion,w_count,w_buy,w_de,w_ar,w_variance,w_mean,w_error,w_hotcity,w_normalcity))
        ...
        sql = 'INSERT INTO procdata.ipdaymessage ( ip,time, duration, query, buy, depature, arrival, variance, mean, error,hotcity,normalcity) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s)'
        if all_count > 0:
            self.cursor.execute(sql,('all',dataa,0,all_count,all_buy,all_de,all_ar,0.0,0.0,all_er,all_hot,all_normal))
        else:
    ...
    def clear_cache(self,data):  
        ...
        if w_query >= 2:
            ...
            w_mean = 0
            w_variance = 0
        returndict = {'count':w_query,'duration':w_duration,'mean':w_mean,'variance':w_variance,'error':data['error']}
        return returndict
    def makecrawlerip(self,data):
        ...
        delta = datetime.timedelta(days = 1)
        date2 = date1 + delta
        sql = 'SELECT ip FROM procdata.sessiondiv where `starttime`>= %s AND `starttime` < %s AND `svmclass` = 2'
    ...
    def MakePositiveSample(self,a):
        ...
        for i in range(len(iplist)):
            ...
            self.cursor.execute(sql,(datetime.datetime.strftime(tempdate1,timeformat2),datetime.datetime.strftime(tempdate2,timeformat2),tempip))
            ...
            if threedaycount < 500.0*len(result2):
                ...
                self.cursor.execute(sql,(datetime.datetime.strftime(tempdate1,timeformat2),datetime.datetime.strftime(tempdate2,timeformat2),iplist[i]))
